    ...
    try:
        for page in range(1, effective_max_pages + 1):
            ...
            if not rows:
                ...
                if stopped_reason in {"blocked_http_429", "blocked_kpsdk", "blocked_access_denied"}:
                    raise RealEstateBlockedError(stopped_reason)
                if stopped_reason in {"render_timeout", "blank_render", "unknown"}:
        ...
        return {
            ...
            "scan_status": "valid_empty_result" if stopped_reason == "no_results" else ("technical_failure" if errors else "ok"),
            "page_state": stopped_reason if stopped_reason in {"no_results", "render_timeout", "blank_render", "unknown"} else None,
